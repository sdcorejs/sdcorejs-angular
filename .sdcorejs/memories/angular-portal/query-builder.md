...
- **Field-to-field comparison is supported behind `comparisonMode="value-or-field"`** (added 2026-06-10). Default `value-only` preserves literal-only behavior. Internal rule state uses `valueSource: 'literal' | 'field'` + `compareField`; public `Filter` uses the canonical utils shape `{ dataType: 'field', data: '<rightField>' }`. Candidates are same-type fields, optionally constrained by matching `compareGroup`; compound/no-data operators stay literal/no-value only.
