...
- [ ] Repair the 15 current v19 Karma baseline failures in Chip, ChipCalendar, Inform, AnchorNav, QuerySavedFiltersMenu, and order-dependent form-builder drag/drop specs.
- [ ] Normalize `GenericSelectComponent` output types, then migrate its three `EventEmitter` outputs with Angular safe migration.
- [ ] Revisit the 180 skipped setter/aliased inputs in focused groups with contract tests before attempting manual signal migration.
