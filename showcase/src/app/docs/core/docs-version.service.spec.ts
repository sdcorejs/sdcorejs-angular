...
describe('DocsVersionService', () => {
  ...
  it('loads versions from the configured base URL and exposes semantic groups', async () => {
    ...
    expect(service.latestVersion()).toBe('22.2.5');
    expect(service.versionGroups().map(group => group.label)).toEqual(['22.x', '21.x', '20.x', '19.x']);
  ...
  it('upgrades a stored pre-inception Angular 22 version to 22.2.5', async () => {
    storage.getItem.and.returnValue('22.1.9');
    const pending = service.load();
    http.expectOne('https://example.test/app/docs/versions.json').flush(manifest);

    await pending;

    expect(service.selectedVersion()).toBe('22.2.5');
  });
  ...
  it('falls back to latest for an invalid route version and exposes a notice', async () => {
    ...
    expect(await pending).toBe('22.2.5');
  ...
  it('persists selection and preserves the current documentation route', async () => {
    const pending = service.select('22.2.5');
    http.expectOne('https://example.test/app/docs/versions.json').flush(manifest);
    await pending;

    expect(storage.setItem).toHaveBeenCalledWith('sdcorejs.docs.version', '22.2.5');
    expect(router.navigateByUrl).toHaveBeenCalledWith('/v/22.2.5/components/button/api');
  });
