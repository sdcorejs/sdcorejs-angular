...
In the body, delete the operator block + both `c-pop-section-label` rows. Replace the body open (lines ~89-106, from `<div class="c-pop-body" ...>` through the removed `Giá trị` label) so the body starts directly with the value `@if`:
...
(The `@if (_editOp === 'BETWEEN')` and the rest of the value ladder stay; just remove the wrapping `<div>` + `<div class="c-pop-section-label">Giá trị</div>` that previously sat between `c-pop-body` and the value `@if`. Ensure the closing `}` / `</div>` count stays balanced — the value ladder's outer `@if (!isNoDataOperator(_editOp))` now wraps the kind branches directly.)

Add placeholders to the single-value controls in that ladder:
- `sd-select` (values/lazy-values, line ~174): add `placeholder="Chọn giá trị"`.
- `sd-date` (line ~187): add `placeholder="Chọn giá trị"`.
- `sd-datetime` (line ~193): add `placeholder="Chọn giá trị"`.
- `sd-input-number` (line ~199): add `placeholder="Nhập giá trị"`.
- `sd-input` (line ~205): add `placeholder="Nhập giá trị"`.
    ...
    <ng-template matMenuContent let-field="field">
      <div class="c-value-popover-body" (click)="$event.stopPropagation()">
        @if (field.kind === 'values' || field.kind === 'lazy-values') {
          ...
          <sd-datetime size="sm" [autoId]="'qb-valpop'" placeholder="Chọn giá trị" (sdChange)="commitValuePopover($event)"></sd-datetime>
        }
      </div>
    </ng-template>
  </mat-menu>
...
- **Spec coverage (compact popover):** operator in header + no "Điều kiện" label (Task 2); drop "Giá trị" label + placeholders (Task 2); remove Áp dụng + auto-apply on close, no emit (Task 1); no-data → empty body (existing `@if (!isNoDataOperator)` retained, Task 2). **Search button (B):** far right (Task 3) + neutral style (Task 3).
