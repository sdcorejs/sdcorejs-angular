# query-bar inline alignment + BETWEEN dual + selected-display fixes â€” Implementation Plan
...
**Architecture:** Pure query-bar changes â€” `::ng-deep` overrides scoped under `.c-token` for alignment + bare-select value display, a small dual-render branch in the inline build value step and inline edit value editor, and two TS helpers (`setBuildRangeFrom` / `setBuildRangeTo`). No edits to `sd-select` / `sd-date` / `sd-datetime` core.
...
Branch `query-bar`. Each commit must start with the ticket prefix `SM-00:` (pre-receive hook requires it â€” branch convention). Keep Vietnamese `// why:` comments.

## File Structure

- Modify: `projects/sdcorejs-angular/components/query-bar/src/query-bar.component.scss` â€” `::ng-deep` overrides under `.c-token`.
- Modify: `projects/sdcorejs-angular/components/query-bar/src/query-bar.component.ts` â€” `setBuildRangeFrom` / `setBuildRangeTo` + a small inline-edit range setter wrapper.
- Modify: `projects/sdcorejs-angular/components/query-bar/src/query-bar.component.html` â€” BETWEEN dual branches in the build value step + the inline edit value editor.
- Modify: `projects/sdcorejs-angular/components/query-bar/src/query-bar.component.spec.ts` â€” DOM + behavioral tests.

---

## Task 1: Chip-level CSS overrides â€” alignment + bare-select value visibility
...
Run the query-bar spec. Expected: FAIL â€” `sd-select.sd-bare` either renders without `.mat-mdc-select-value` exposed in the test fixture OR the structural query doesn't find it under the chip class. (Either way, the test pins the contract that the chip must render the bare select with a value element.)

- [ ] **Step 3: Add the `::ng-deep` overrides under `.c-token`**

In `query-bar.component.scss`, place the new block immediately after the existing `.c-token` rules (search for `// Inline mode (GitLab-style token builder) â€” flat completed chips` and add at the end of that section, before the next `---` divider):
...
.c-token ::ng-deep {
  ...
  // sd-select bare's selected text â€” make it visible (primary, weight, ellipsized) and avoid
...
Run the query-bar spec. Expected: FAIL â€” `component.setBuildRangeFrom is not a function`.
...
## Task 3: Template â€” render dual pickers for BETWEEN date/datetime
...
```ts
  describe('inline BETWEEN dual pickers (date / datetime)', () => {
    ...
    it('date BETWEEN build value step renders two sd-date (Tá»« / Äáº¿n)', () => {
      fixture.componentRef.setInput('fields', [dateField]);
      component.beginBuild(dateField); // single op BETWEEN â†’ straight to value step
      fixture.detectChanges();

      const dates = fixture.nativeElement.querySelectorAll('.c-token-building sd-date');
      expect(dates.length).toBe(2);
      expect((dates[0] as HTMLElement).getAttribute('placeholder')).toBe('Tá»«');
      expect((dates[1] as HTMLElement).getAttribute('placeholder')).toBe('Äáº¿n');
    });

    it('datetime BETWEEN build value step renders two sd-datetime (Tá»« / Äáº¿n)', () => {
      ...
      expect((dts[0] as HTMLElement).getAttribute('placeholder')).toBe('Tá»«');
      expect((dts[1] as HTMLElement).getAttribute('placeholder')).toBe('Äáº¿n');
    });
...
Run the query-bar spec. Expected: FAIL â€” only one `<sd-date>` rendered (current branch always renders a single picker for date/datetime).
...
```html
            ...
            <span class="c-token-value c-token-value-edit">
              @if (_b.operator === 'BETWEEN') {
                <sd-date #bPicker bare size="sm" autoId="qb-build-value-from" placeholder="Tá»«"
                  [model]="$any(_b.value)?.from" (sdChange)="setBuildRangeFrom($event)"></sd-date>
                <span class="c-token-dash" aria-hidden="true">â€”</span>
                <sd-date bare size="sm" autoId="qb-build-value-to" placeholder="Äáº¿n"
            ...
            <span class="c-token-value c-token-value-edit">
              @if (_b.operator === 'BETWEEN') {
                <sd-datetime #bPicker bare size="sm" autoId="qb-build-value-from" placeholder="Tá»«"
                  [model]="$any(_b.value)?.from" (sdChange)="setBuildRangeFrom($event)"></sd-datetime>
                <span class="c-token-dash" aria-hidden="true">â€”</span>
                <sd-datetime bare size="sm" autoId="qb-build-value-to" placeholder="Äáº¿n"
...
The `#bPicker` ref stays on the FIRST picker (Tá»«) so the existing `buildPicker()?.open()` auto-opens it on entering the value step.
...
- **Spec coverage:**
  - A. Chip alignment â†’ Task 1 (`::ng-deep` overrides under `.c-token`).
  - B. sd-select bare selected display â†’ Task 1 (`.mat-mdc-select-value` styled with primary color + min-width + ellipsis).
  - C. BETWEEN dual for date/datetime â†’ Tasks 2 (TS helpers) + 3 (template build value step + inline edit value editor + dash SCSS).
  - D. Tests â†’ each task ships its own DOM + behavioral assertions; Task 4 confirms full build + cross-component sweep.
- **Type consistency:** `setBuildRangeFrom(v: unknown)` / `setBuildRangeTo(v: unknown)` consistent across TS + template. `setFilterRangeFrom(i, v)` / `setFilterRangeTo(i, v)` reused from existing popover wiring â€” verified by the prior plan that introduced them.
- **Risk:** the `::ng-deep` overrides apply to ALL mat-form-fields inside `.c-token` â€” that's exactly the bare pickers (and nothing else lives inside a token), so scope is correct. The `auto-commit on both-set` behavior in `setBuildRangeTo` is the same approach the inline-value-chip already uses for seamless BETWEEN (`commitRange()` on blur with both ends populated) â€” consistent UX.
- **Out of scope reminder:** no changes to `sd-select` / `sd-date` / `sd-datetime` source. If the bare-select value still does not appear after Task 1, the Minor follow-up is `lazyItemsFor`'s memoization â€” verify it resolves a real array (not a fn) before the select renders; do NOT modify `sd-select` itself.
