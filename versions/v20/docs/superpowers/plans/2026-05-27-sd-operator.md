...
```ts
  // Inner SVG markup dùng khi operator chưa chọn — hình phễu (funnel) trung tính.
  ...
  // <svg> viewBox 0 0 24 24 + stroke=currentColor rồi bypass sanitizer (nguồn là hằng
  // số nội bộ, không phải input người dùng) để Angular không strip thẻ svg con.
  #svg(inner: string): SafeHtml {
    return this.#sanitizer.bypassSecurityTrustHtml(
      `<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${inner}</svg>`,
    );
...
```ts
  // SVG icon của operator đang chọn (fallback hình phễu khi chưa chọn).
  ...
  // SVG icon cho một operator bất kỳ trong menu.
...
```ts
  ...
  // why: OPERATORS.icon là inner SVG (path/line/rect), không phải ligature. Bọc bằng
  // <svg> + bypass sanitizer (nguồn là hằng số nội bộ, không phải input người dùng).
  #svg(inner: string): SafeHtml {
    return this.#sanitizer.bypassSecurityTrustHtml(
  ...
  /** SVG icon cho một operator option (đã kèm sẵn `icon` trong items). */
  operatorIcon(op: { icon: string }): SafeHtml {
    return this.#svg(op.icon);
  }
```
