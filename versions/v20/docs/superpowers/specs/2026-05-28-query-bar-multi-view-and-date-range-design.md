# query-bar â€” multi "head +N" view + sd-date-range for BETWEEN
...
2. **BETWEEN renders two pickers.** date / datetime BETWEEN currently shows 2 bare pickers (Tá»« + Äáº¿n) plus a dash. The library already ships `sd-date-range` (single range component with one open panel + two-end model). Switching to it simplifies the chip + matches the date-range pattern used elsewhere.

## Goal

- **A.** Chip sd-select multi view shows the first selected item's `displayField` + ` +N` (N = remaining count) â€” single line, ellipsizable. 1 selected â†’ just the label. 0 â†’ empty.
...
- Popover-mode sd-select still renders its default display (unchanged) â€” it uses `chipValueText` on the closed chip.
...
- Template: `@if (viewed()) { <sd-view [value]=... [display]="formatted()" /> } @else { <existing form-field> }`. `formatted()` returns a `"dd/MM/yyyy â†’ dd/MM/yyyy"` string (or empty).
- SCSS: replicate the `:host(.sd-bare)` block (same MDC bÃ³c-khung rules) and `:host(.sd-viewed) { padding-top: 0 }`. Reuse the documented WHY comments from sd-date so the rationale is consistent.
- Public `open = () => â€¦` calling the internal `MatDateRangePicker`'s open via a `viewChild` (`MatDateRangePicker` exposes `.open()`).
...
(no `[viewed]` â€” build is always editable. `#bPicker` keeps the auto-open viewChild target.)
...
(Existing `setFilterRangeFrom` / `setFilterRangeTo` and `setBuildRangeFrom` / `setBuildRangeTo` become unused once dual pickers go â€” remove after grep confirms zero references.)
...
- `projects/sdcorejs-angular/forms/date-range/sd-date-range.md` â€” document the new `bare`, `viewed` inputs + the public `open()` method, when each is used.
- `projects/sdcorejs-angular/components/query-bar/sd-query-bar.md` (or `HANDOFF.md` if that's the kept doc) â€” note the chip multi "head +N" rendering rule and the BETWEEN-via-sd-date-range pattern (including the datetime downgrade).
- `CLAUDE.md` (root vn-angular) â€” refresh the "Recent work" / open follow-ups bullet for query-bar with this iteration's bullet.

## Testing (TDD)

- **Multi view:** mount a chip with `kind: 'values'`, operator `IN`, data `['a','b','c']`. Assert `.c-token sd-select` text content matches the displayField label of `a` + ` +2`. Edge: data length 1 â†’ just label, no `+0`. Empty array â†’ empty text.
- **Single chip unaffected:** EQUAL operator with one value still shows that value's label normally (no `+`).
- **sd-date-range bare:** mount `<sd-date-range bare>` â†’ host has `.sd-bare` class; the form-field strips outline (assert `.mdc-notched-outline` invisible via display:none or absence).
- **sd-date-range viewed:** mount with viewed=true â†’ renders `<sd-view>` (no form-field).
- **sd-date-range open():** calling `component.open()` invokes the internal `MatDateRangePicker.open()` (spy).
- **Chip BETWEEN date:** completed chip with `BETWEEN` + `{from,to}` â†’ renders ONE `<sd-date-range>` (not two `<sd-date>`).
- **Chip BETWEEN datetime:** same as date â†’ ONE `<sd-date-range>` (datetime downgraded).
...
- Popover mode (its BETWEEN uses 2 controls inside a panel â€” leave as-is).
