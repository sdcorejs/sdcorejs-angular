...
Thư vi�!n `@sdcorejs/angular` hi�!n có 17 spec files cover �ược pipes, utility extensions, storage service và table. Còn ~22 components, 14 forms, 6 directives, 9 services, modules/handlers/interceptors chưa có test. File `.md` cho từng API �ã rất chi tiết (button/badge/avatar là mẫu t�t) nhưng không ��ng �ều.

Mục tiêu: tĒng test coverage cho 10 file primitives quan trọng nhất + rà soát + cải thi�!n `.md` của 10 file �ó �Ồ agent/skill tương lai dùng �ược nhất quán.
...
- Thay ��"i cấu trúc thư mục, ��"i tên class/selector.
...
**TestBed-driven full integration**. M�i component/form dùng `TestBed.createComponent` + `ComponentFixture`, render template, query DOM, simulate event. Test class + template trong cùng spec.

**Lý do chọn**:
1. User chọn Full coverage �  cần cover cả template behavior.
2. `<sd-button>`, `<sd-badge>` có `booleanAttribute` coerce + boolean shortcut + host class bindings � ch�0 TestBed bắt �ược.
3. Forms dùng FormControl/FormGroup integration �  cần render template �Ồ wire `[(ngModel)]` hoặc `[form]`.
4. Directives cần host component anyway.

**Trade-off chấp nhận**: setup nặng hơn class-only, m�i test ~50-200ms. V�:i 10 file � 15-25 test = ~250 test, t�"ng chạy ư�:c tính < 60s.

### 3.2. File location & convention

Đặt file `*.spec.ts` cạnh source file (theo pattern hi�!n hữu của repo):
...
projects/sdcorejs-angular/
  components/
    button/src/
      button.component.ts
      button.component.spec.ts        � � M�aI
    badge/src/
      badge.component.ts
      badge.component.spec.ts         � � M�aI
    avatar/src/
      avatar.component.ts
      avatar.component.spec.ts        � � M�aI
    anchor/src/components/anchor/
      anchor.component.ts
      anchor.component.spec.ts        � � M�aI
  forms/
    input/src/
      input.component.ts
      input.component.spec.ts         � � M�aI
    checkbox/src/
      checkbox.component.ts
      checkbox.component.spec.ts      � � M�aI
    switch/src/
      switch.component.ts
      switch.component.spec.ts        � � M�aI
    label/src/
      label.component.ts
      label.component.spec.ts         � � M�aI
  directives/src/
    sd-tooltip.directive.ts
    sd-tooltip.directive.spec.ts      � � M�aI
    sd-mobile.directive.ts
    sd-mobile.directive.spec.ts       � � M�aI
  testing/                            � � M�aI
    test-utils.ts                     � � M�aI
```
...
- `it` ngắn gọn, m�!nh �ề tiếng Anh: `"emits click once per rapid press"`, `"applies .sd-disabled when disabled is true"`.
...
Tạo `projects/sdcorejs-angular/testing/test-utils.ts` � **không** export ra `public-api.ts`, ch�0 dùng n�"i b�" cho test.

API:

```ts
// Tạo fixture + tự gọi detectChanges() lần �ầu.
...
- `setInput` quan trọng vì sd-angular dùng signal inputs (Angular 19) � pattern khác `@Input` c�" �iỒn.
- `queryByCss` throw rõ ràng giúp debug fail nhanh hơn `By.css` trả null.

**Fallback**: nếu component/form có quirk khiến helper khó dùng (ví dụ cần `TestBed.overrideComponent`), spec �ó có thỒ dùng `TestBed` trực tiếp.

### 3.4. Test scope per file

Phân nhóm theo ��" phức tạp:

#### Đơn giản (5-10 tests/file)

**`label.component.spec.ts`** (`SdLabel`):
- Setter `@Input` cho `label`/`description`/`helperText` truyền vào �  render �úng text.
- `required` coerce: `true`, `''`, `false`, `null`, `undefined` �  �úng boolean.
- Render dấu `*` khi required = true.
- Render tooltip khi có `helperText`.

**`sd-mobile.directive.spec.ts`** (`SdMobileDirective`):
- Mock `SdUtilities.isMobile()` �  `true`: template �ược render.
- Mock `SdUtilities.isMobile()` �  `false`: template KH�NG render.
...
- Free text: `"Nguy�&n VĒn An"` �  render initials `"NA"` (computed).
...
- `effect`: khi `src` ��"i từ broken URL �  URL khác, `#imageError` �ược reset.
- `size` input mặc ��9nh 32, custom �  render �úng width/height.
...
- `click` output: `stopPropagation` �ược gọi r�i emit.
- `type='round'` ch�0 render text, không icon.
- `type='tag'` render tinted background.
- `type='icon'` render row icon + text.
- Computed `effectiveColor` �úng theo input.
...
- Host class `.sd-disabled`/`.sd-loading`/`.sd-block` áp �úng.
- Click throttle 300ms: bắn 3 click trong 100ms �  ch�0 emit 1 lần (dùng `fakeAsync` + `tick`).
...
- `ngOnDestroy` unsubscribe (gọi `destroy` r�i bắn click �  không emit).

**`switch.component.spec.ts`** (`SdSwitch`):
- `disabled = true` �  `formControl.disable()` �ược gọi.
- `disabled = ''` (bare attribute) coerce true.
- Model setter: set `true` �  `formControl.value === true` mà KH�NG emit `modelChange`.
...
- `name` setter: bỏ qua falsy, giữ uuid mặc ��9nh.
- FormGroup integration: pass FormGroup �  `addControl(name, formControl)` �ược gọi; destroy �  `removeControl` �ược gọi.

**`checkbox.component.spec.ts`** (`SdCheckbox`):
- Tương tự switch: disabled, model, formControl integration, modelChange/sdChange.
- `inlineError` setter: set string �  custom validator emit error �úng tên `inlineError` v�:i message �ó.
- `inlineError` clear �  validator �ược xóa.
...
- `type='password'`/`'number'`/`'email'`/`'text'` �úng attribute trên input.
...
- `readonly = true` �  attribute `readonly` xuất hi�!n.
- `blurOnEnter`: keydown Enter �  blur �ược gọi.
...
- Custom validator wiring nếu có `inlineError` (nếu quá phức tạp, có thỒ tách thành ticket riêng � quyết ��9nh trong phase writing-plans).

**`anchor.component.spec.ts`** (`SdAnchor`):
- Render anchor list từ `contentChildren(SdAnchorItem)`.
- Default `activeSectionId` = id của section �ầu tiên (sau `afterNextRender`).
- Scroll wrapper �  cập nhật `activeSectionId` �úng theo section �ang trong viewport (mock scroll event + giả lập rect).
...
- mouseenter sau `sdTooltipDelay` ms �  overlay �ược attach.
...
- `sdTooltipPosition='top'/'bottom'/'left'/'right'` �  tạo �úng `ConnectionPositionPair` (test via `withPositions` spy hoặc inspect overlay config).
- `sdTooltipColor` truyền vào �  background tooltip �úng màu.
- `destroyRef` cleanup: directive destroy �  overlay disposed, `activeTooltip` static reset về null.
- Static `activeTooltip` b�9 share giữa instance � verify ch�0 1 active tại 1 thời �iỒm.

### 3.5. Rà soát + cải thi�!n `.md`

#### Checklist hoàn ch�0nh cho m�i `.md`

1. Frontmatter: Type, Selector/Class, Standalone, Change detection, Import path, Library version.
2. One-line purpose.
3. When to use (�0�3 bullet).
4. When NOT to use (�0�3 bullet).
5. Inputs table (Name, Type, Default, Notes).
6. Outputs table.
7. Content projection (ghi "None" nếu N/A).
8. Visual cues (bỏ qua v�:i directive).
9. Examples (�0�3, có cả tiếng Vi�!t thực tế trong app).
10. Anti-patterns (�0�3 bullet).
11. Related (link t�:i các API liên quan).
...
14. **Code mẫu chi tiết theo trường hợp**:
    - M�i Example phải có code snippet �ầy �ủ (HTML + TS nếu cần) thay vì ch�0 tag r�ng.
    - M�i case dùng �ặc bi�!t có **code mẫu + di�&n giải** �i kèm: 1-2 câu giải thích "tại sao dùng cấu hình này", "behavior trong runtime ra sao", "khác gì v�:i case khác".
    - Edge cases / null behavior nên có code mini-snippet minh hoạ.
    - Anti-patterns: snippet "Đừng làm vậy" + 1 câu lý do; kèm "Thay vào �ó hãy dùng:" + snippet �úng.
    - Directive cleanup / side-effect: snippet minh hoạ k�9ch bản �ặc trưng (vd tooltip static `activeTooltip`).
    - Form integration: snippet show cả 3 cách dùng (template-driven `[(ngModel)]`, NgForm, reactive FormGroup) + di�&n giải khi nào chọn cái nào.
    - Quy tắc: snippet **chạy �ược** (�ủ import/binding), không pseudo-code. Di�&n giải �0� 2 câu, tập trung "tại sao" thay vì "cái gì".

#### Cách thực hi�!n

- M�i file `.md` trong scope: �ọc `.md` hi�!n tại + �ọc source �  ��i chiếu checklist.
- Tạo gap report (mục 6 dư�:i �ây) li�!t kê m�i file thiếu gì.
- B�" sung trực tiếp vào `.md` (không tạo file m�:i).
- Cải thi�!n wording/format hi�!n hữu:
  ...
  - Vi�!t hoá nhất quán: thuật ngữ kỹ thuật giữ tiếng Anh (`OnPush`, `signal`, `FormGroup`), n�"i dung di�&n giải dùng tiếng Vi�!t � không tr�"n 2 thứ tiếng trong cùng 1 câu.
  - Sửa typo, câu cụt, ký tự lạ.
  - Loại bỏ comment thừa tiếng Vi�!t tr�"n trong code snippet.
  - Snippet phải compile �ược trong Angular 19 + sd-angular hi�!n tại.

## 4. Tooling

- **Test runner**: Karma + Jasmine (�ã có sẵn).
- **Script chạy**: `npm run test:ci` � `ng test sdcorejs-angular --watch=false --browsers=ChromeHeadless`.
- **Code coverage**:
  - `karma-coverage` �ã có trong devDeps. B�" sung config trong `karma.conf.js` (tạo m�:i nếu chưa có) �Ồ xuất report.
  - Threshold mục tiêu cho 10 file trong scope: �0� **80% line**, �0� **70% branch**.
  - File phức tạp (input, anchor, tooltip) có thỒ giảm sàn xu�ng 70% line / 60% branch � ch�t cụ thỒ khi �o thực tế.
- **Lint**: `npm run lint` � phải pass cho file spec m�:i.

## 5. Acceptance criteria

1. 10 spec file m�:i + 1 file `testing/test-utils.ts` t�n tại �úng �ường dẫn (mục 3.2).
2. `npm run test:ci` pass 100% (không failing, không pending).
3. 10 file `.md` �ã �ược audit theo checklist 14 mục; nếu có gap �ã �ược b�" sung; nếu không gap thì ghi "no gap" trong gap report (mục 6).
4. Wording/format `.md` �ược cải thi�!n theo quy tắc tại mục 3.5.
5. Gap report �ược commit cùng design doc này.
6. Không break existing test (17 spec hi�!n hữu vẫn pass).
7. Không thay ��"i source `.ts` của 10 file (trừ khi phát hi�!n bug rõ ràng � nêu trong commit message).
8. Coverage threshold �ạt yêu cầu (mục 4).
9. Tất cả thay ��"i commit trên 1 branch riêng (�ề xuất `feature/plan-1-core-ui-tests`).

## 6. Gap report template

Sẽ �iền khi thực hi�!n. Format:

```markdown
### sd-button.md
- [x] Frontmatter �ầy �ủ.
- [x] One-line purpose.
- [x] When to use �0�3 bullet.
- ...
- [ ] Example 4: thêm case button trong reactive FormGroup submit.
- [ ] Anti-pattern code snippet thiếu.
- [ ] Wording: câu "Throttled to 300ms (leading edge)" lặp ý 2 lần � g�"p.
```
...
M�i plan có spec + plan riêng theo quy trình superpowers.
