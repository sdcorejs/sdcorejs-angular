...
## When NOT to use
- ❌ For dev logs (`console.log/warn/error`) — these are not UX. Leave VI / EN literals as-is and mark the line with `// @i18n-ignore` so the guard script ignores them. The marker covers the line it sits on or the single line directly below it — a multi-line concatenated string needs one marker per Vietnamese line (see `layout.service.ts` for the pattern).
...
```typescript
// Built-in languages. Owned by `@sdcorejs/utils/models` — NOT exported from this
// entry point: import { Language } from '@sdcorejs/utils/models'.
...
> **Why does `Language` live outside this entry point?** Both `@sdcorejs/angular/configurations` and `@sdcorejs/angular/i18n` need the type, and `i18n` already imports from `configurations` — a type owned by `i18n` would create a cross-entry-point cycle (ng-packagr build failure). The type is owned by `@sdcorejs/utils/models` (with `SUPPORTED_LANGUAGES` in `@sdcorejs/utils/constants`), so both entry points consume it without a cycle. The former `@sdcorejs/angular/models` re-export entry point is removed.

### Constants
```typescript
// SUPPORTED_LANGUAGES (ordered list of built-in languages) is owned by
// `@sdcorejs/utils/constants` — import it from there, not from this entry point.
...
## Related
- `@sdcorejs/utils/models` / `@sdcorejs/utils/constants` — own the `Language` type + `SUPPORTED_LANGUAGES` constant. Imported by both `configurations` and `i18n`.
- `@sdcorejs/angular/configurations` (`SD_CORE_CONFIGURATION`) — DI token where `language` is set.
- `@sdcorejs/utils/models` (`MaybeAsync`) — used elsewhere but **not** by i18n (custom language is sync-only by design).
- `@sdcorejs/angular/utilities/extensions` (`SdUtilities.upload`) — example of the DI-less hardcode pattern (see Example #7).
