...
| `icon.model.ts` | `MaterialIconFontSet` (type), `DefaultMaterialIconFontSet` (const) | local (Angular-only) | Allowed Material icon font sets (`'material-icons-outlined'`, `'material-symbols-outlined'`, ...) and the library default (`'material-icons-outlined'`). |
