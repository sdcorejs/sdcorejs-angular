...
## One-line purpose
Page-level banner / alert (bÃ¡o lá»—i / cáº£nh bÃ¡o / thÃ´ng tin) neo trÃªn page â€” a presentational notice with color, icon, title, description, optional action link and dismiss button. Presentational only: the consumer positions it.

## When to use
- A persistent page-level notice the user should read before continuing (lá»—i táº£i dá»¯ liá»‡u, cháº¿ Ä‘á»™ chá»‰ Ä‘á»c, thÃ´ng bÃ¡o báº£o trÃ¬)
...
## When NOT to use
- For a transient, auto-dismissing notification â†’ use `NotifyService` (toast)
- For a short inline status label â†’ use `<sd-badge>`
- For a blocking confirmation the user must answer â†’ use `<sd-modal>` / `ConfirmService`
...
| `hideIcon` | `boolean` | `false` | `transform: booleanAttribute` â€” bare attribute = true. Suppress the icon entirely (no auto-map). |
...
| `closable` | `boolean` | `false` | `transform: booleanAttribute` â€” bare attribute = true. Show the `Ã—` dismiss button. |
...
> Boolean color shortcuts take priority over `color` (precedence: primary â†’ secondary â†’ info â†’ success â†’ warning â†’ error â†’ `color` input).

## Outputs
| Name | Type | Notes |
| --- | --- | --- |
| `sdClosed` | `output<Event>` | Fired when the `Ã—` button is clicked. The component self-hides (uncontrolled â€” flips an internal `dismissed` signal) AND emits. The host calls `event.stopPropagation()` before emitting. |
| `sdAction` | `output<Event>` | Fired when the `actionLabel` text-link is clicked. The host calls `event.stopPropagation()` before emitting. (Not fired by the `[sdInformAction]` slot â€” wire your own handler there.) |
...
## Behavior
- **Uncontrolled close**: clicking `Ã—` sets an internal `dismissed` signal and the banner removes itself from the DOM, in addition to emitting `sdClosed`. There is no `[open]` input to re-show it â€” recreate / re-render the component to bring it back.
...
- **ARIA role**: `error` / `warning` render with `role="alert"` (assertive); other colors use `role="status"` (polite). Decorative icons (status icon + close `Ã—`) are `aria-hidden`.
...
<sd-inform warning title="Cháº¿ Ä‘á»™ chá»‰ Ä‘á»c" description="Báº¡n khÃ´ng cÃ³ quyá»n chá»‰nh sá»­a.">
  <button sdInformAction>YÃªu cáº§u quyá»n</button>
</sd-inform>

<sd-inform info title="Äiá»u khoáº£n" [description]="longText" [lineClamp]="3"></sd-inform>
```

## Anti-patterns
- Using `<sd-inform>` as a transient toast â€” it is persistent and page-anchored; for fire-and-forget notifications use `NotifyService`
- Hardcoding user-facing strings into `title` / `description` / `actionLabel` â€” route them through `I18nService`
- Cramming a long sentence into `title` â€” keep the title short and put the detail in `description`
- Using it where a short inline status fits â€” use `<sd-badge>` instead

## Related
- `<sd-badge>` â€” short inline status / label indicator
- `NotifyService` â€” transient toast notifications
- `<sd-modal>` / `ConfirmService` â€” blocking confirmation dialogs
