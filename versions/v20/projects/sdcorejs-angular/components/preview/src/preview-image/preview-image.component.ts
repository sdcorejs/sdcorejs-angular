...
@Component({
  ...
  host: {
    ...
    // to swap CSS custom properties â€” keeps the theme variant declarative and
...
export class SdPreviewImage implements OnDestroy {
  ...
  // Theme variant â€” drives [data-theme] host attribute. Default 'dark' preserves
  ...
  // ==========================================
  constructor() {
    // Äá»“ng bá»™ #isFullscreen vá»›i tráº¡ng thÃ¡i thá»±c cá»§a browser (sá»± kiá»‡n 'fullscreenchange' fire
    // báº¥t ká»ƒ user thoÃ¡t fullscreen báº±ng cÃ¡ch nÃ o: nÃºt ThoÃ¡t, Esc, F11, programmatic).
  ...
  toggleFullscreen(): void {
    // WHY: tráº¡ng thÃ¡i #isFullscreen Ä‘Æ°á»£c sync qua sá»± kiá»‡n 'fullscreenchange' (xem constructor)
    // â€” single source of truth. KhÃ´ng cáº§n .then(set(true/false)) á»Ÿ Ä‘Ã¢y ná»¯a.
  ...
  async retryActive(): Promise<void> {
    ...
    // Per-image loading: chá»‰ Ä‘Ã¡nh dáº¥u record nÃ y loading, KHÃ”NG Ä‘á»¥ng stage().
    // Nhá» váº­y nav arrows + thumbnails váº«n render trong lÃºc retry, khÃ´ng flicker.
    ...
    if (refreshed) {
      ...
      // Normalize tráº£ null â†’ giá»¯ record cÅ© nhÆ°ng táº¯t loading + mark error
      // Ä‘á»ƒ inline placeholder hiá»ƒn thá»‹ (khÃ´ng láº­t cáº£ stage).
      this.#images.update(arr => {
        const next = [...arr];
  ...
  onPointerDown(event: PointerEvent): void {
    ...
    // Bá» qua náº¿u pointer xuáº¥t phÃ¡t trÃªn control tÆ°Æ¡ng tÃ¡c (toolbar / nav / dots / retry button).
    // WHY: stage chá»©a cÃ¡c button con; setPointerCapture Ä‘i cÃ¹ng drag/swipe logic sáº½
    // "Äƒn" click event cá»§a nhá»¯ng button Ä‘Ã³ (nhÆ° nÃºt ThoÃ¡t fullscreen / zoom / rotate).
  ...
  onImageError(): void {
    ...
    // WHY khÃ´ng set stage='error': lá»—i tá»«ng áº£nh khÃ´ng nÃªn áº©n nav/dots/thumb cá»§a
    // cáº£ viewer. Template render placeholder inline ngay trong khung áº£nh khi
    // activeImage().error === true â€” user váº«n cÃ³ thá»ƒ chuyá»ƒn sang áº£nh khÃ¡c.
  ...
  async #normalizeAndLoad(items: PreviewItem[] | null | undefined, startIndex: number): Promise<void> {
    ...
    if (valid.length === 0) {
      ...
      // WHY luÃ´n 'ready' ká»ƒ cáº£ khi áº£nh Ä‘áº§u bá»‹ error: per-image error render
      // inline Ä‘á»ƒ giá»¯ nav + thumb interactive. User váº«n Ä‘iá»u hÆ°á»›ng Ä‘Æ°á»£c sang
      // áº£nh khÃ¡c.
  ...
  #setActive(index: number): void {
    ...
    this.#resetTransform();
    // WHY luÃ´n set 'ready' (khÃ´ng phá»¥ thuá»™c img.error): per-image error giá»
    // render inline trong stage thay vÃ¬ láº­t cáº£ stage sang 'error'. Giá»¯ stage
    // 'ready' Ä‘á»ƒ nav/dots/thumbnail/toolbar khÃ´ng bá»‹ áº©n.
    this.#stage.set('ready');
