...
- As an alternative to `sd-table`'s inline column filters when �0�3 filters are typical.

## When NOT to use

- �R Per-row filter (use the inline column filter on `sd-table`).
- �R Form-style multi-field input (use `<sd-input>` + `<sd-select>` directly).
- �R Single-filter UI (overkill � use one form control).
...
| `showLogicToggle` | `boolean` | `false` | Render AND/OR segmented toggle (only when �0�2 filters). |
| `showClearAll` | `boolean` | `true` | Show "Xóa tất cả (N)" when �0�1 filter. |
...
| `<sd-query-actions-bar>` | `actions-bar.component.*` | Right-pinned toolbar � divider, AND/OR toggle, saved-filters dropdown, clear-all, "Lưu b�" lọc" shortcut (calls `SdQuerySavedFiltersMenu.promptSave()` via viewChild), Search trigger. |
...
- The **bookmark dropdown** lists `SdSavedFilter[]` from `localStorage` under `sd-query-bar:savedFilters:<key>` � pick to apply, click `�` to delete. Empty state: "Chưa có b�" lọc nào." Leading icon is `filter_alt` (matches the saved-filter intent � not a generic "play").
- A separate **"Lưu b�" lọc hi�!n tại"** icon button (`bookmark_add`) sits immediately before the Search trigger. Clicking it prompts for a name and snapshots the current `{filters, logic, search}` into a new entry. The save action is intentionally adjacent to Search so the "name �  save �  search" flow stays in one toolbar zone.
