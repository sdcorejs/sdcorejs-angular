...
- **Hex pattern validation** � built in. Accepts `#RGB`, `#RRGGBB`, `#RRGGBBAA`. Invalid input shows the inline error message "Mã màu HEX không hợp l�! (vd #1565C0)".
...
```html
<sd-input-color
  label="Màu thương hi�!u"
  helperText="Đ�9nh dạng #RRGGBB hoặc #RRGGBBAA"
  [(model)]="brandColor"
  [required]="true">
</sd-input-color>
```
...
## Anti-patterns
- �R Two-way binding to a non-string (number, object) � the model expects a hex string or null/undefined
- �R Passing names like `'red'` / `'blue'` � pattern validator will reject; use a `<sd-select>` if you need named colors
- �R Skipping `[required]` then trying to enforce non-empty downstream � let the built-in required validator do it
