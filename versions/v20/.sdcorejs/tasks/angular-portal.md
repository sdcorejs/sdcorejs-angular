...
## autoid-inspector E2E test export (2026-06-12)

- [x] Add `host` to `SdAutoidInspectorConfiguration` and expose `SD_AUTOID_INSPECTOR_CONFIGURATION`
- [x] Keep public entrypoint limited to component + configuration/token; do not re-export internal AutoId models
- [x] Rename internal AutoId model/type names without the `Sd` prefix
- [x] Show `Tạo E2E test` export section only when backend host is configured
- [x] Add Playwright and Robot Framework ZIP export actions
- [x] Require Robot Framework export to collect QC context in a popup before calling Forge
- [x] Send AutoId Inspector payload/options to Forge backend endpoints
- [x] Cover host visibility, request payload, ZIP download, and backend error handling with focused specs
- [x] Document configuration, backend contract, and security note that AI API key stays in backend env

## remove uuid dependency (2026-06-11)

- [x] Replace direct `uuid` imports/calls in `projects/sdcorejs-angular` with `Utilities.generateUuid()`
- [x] Remove the direct workspace `uuid` dependency from `package.json` / `package-lock.json`
- [x] Verify with `npm run build` and focused upload-file specs
...
- [ ] (future) Wire query-builder value editor to `<sd-inline-text>` when it is rebuilt off the field/operator model - query-builder rebuilt 2026-06-05 using sd-input/sd-select; sd-inline-text wiring still optional
...
- [x] Build lib pass; showcase build pass
...
- [x] Build lib: pass; targeted org-chart specs: pass; showcase build attempted and blocked by unrelated query-builder/form-generic/date-range errors
