...
```
Expected: `✔ Built @sdcorejs/angular/components/query-bar`, no query-bar/forms errors. (`form-generic` failing later is pre-existing and unrelated.)
  ...
  const lazyField = {
    ...
    option: {
      search: (req: { search?: string }) => Promise.resolve(
        [{ id: 'hn', name: 'Hà Nội' }].filter(c => !req.search || c.name.includes(req.search))),
      views: (values: unknown[]) => Promise.resolve([{ id: 'hn', name: 'Hà Nội' }].filter(c => values.includes(c.id))),
...
```
Expected: all four entries `✔ Built ...`, no `error TS` for them. (`form-generic` failing afterward is pre-existing.)
...
- `Thành phố (lazy)` chip searches via the lazy backend through sd-select.
- `Ngày vào` (date) opens the mat-calendar; `Đăng nhập cuối` (datetime) opens the datetime overlay.
