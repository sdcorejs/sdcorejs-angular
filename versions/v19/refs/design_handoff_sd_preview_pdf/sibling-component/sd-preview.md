...
- Carousel-style preview for �0� 8 images (use `thumbnailPosition="dots"`)
...
| `thumbnailPosition` | `'bottom' \| 'right' \| 'left' \| 'top' \| 'dots' \| 'none'` | `'bottom'` | Where the navigator sits. `dots` swaps the strip for compact dot indicators (�0� 8 images). `none` removes the navigator entirely. |
...
| **G** | Error (`stage = 'error'`) | 96�96 rounded-24 danger tile with `broken_image` icon + "Không tải �ược ảnh" + retry button (`refresh` icon, accent background). |
...
- **Click + drag** when zoom �0� 100%: horizontal swipe �0� 40 px �  next / previous slide.
...
<sd-button (click)="preview.open([row.avatarUrl])">Xem ảnh �ại di�!n</sd-button>
```
...
### 5. Carousel-style with dots indicator (�0� 8 images)
...
- �R Creating one `<sd-preview-image>` per row � declare ONE in the page and call `.open(...)` per click.
- �R Passing PDF / non-image URLs � fetched as `<img>` they show as broken; non-image `File`s are dropped silently.
- �R Trying to bind `[images]` directly � there is no `images` input, always use `open(...)`.
- �R Calling `open()` from inside `ngOnInit` of a component that may be destroyed before the modal opens � the in-flight blob fetches will still resolve and leak handles. Gate on user interaction.
- �R Reading `activeImage()` from outside the template � it's a computed signal; use the `activeIndexChange` output instead.
- �R Mixing `thumbnailPosition="dots"` with > 20 images � dots become illegible. Switch to `bottom`.
