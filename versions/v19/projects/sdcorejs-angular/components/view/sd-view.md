...
<sd-section title="Thông tin chung">
  <div class="row">
    <div class="col-md-6">
      <sd-view label="Tên hợp ��ng" [display]="contract.name"></sd-view>
    </div>
    <div class="col-md-3">
      <sd-view label="Ngày bắt �ầu" [display]="contract.startDate | date:'dd/MM/yyyy'"></sd-view>
    </div>
    <div class="col-md-3">
...
## Anti-patterns
- �R Forgetting `[display]` binding � required input, will fail at compile time. Use `[display]="value ?? ''"` if value can be null.
- �R Building view-mode forms by composing `<sd-view>` instead of toggling `[viewed]` on the form components � the form components already render in read-only mode and keep validation hooks consistent.
- �R Rendering raw HTML in `[display]` � it is plain text. For HTML use `<ng-template #sdValue>` with a sanitizer (or use `<sd-preview>` for rich text).
- �R Using `<sd-view>` for very long text without truncation � wrap value in your own template to add ellipsis/expand UX.
- �R Passing a heavy object to `[value]` and another to `[display]` that contradict each other � keep `display` derived from `value` for clarity.
