...
<div style="height: 480px;">
  <sd-splitter orientation="horizontal">
    ...
    <sd-splitter-panel [size]="3" unit="flex">
      <div class="main">N�"i dung chính (weight 3)</div>
    </sd-splitter-panel>
  </sd-splitter>
</div>
```
...
<div style="height: 100vh;">
  <sd-splitter orientation="vertical">
    <sd-splitter-panel [size]="64" unit="px">
      <header>Header � 64px c� ��9nh</header>
    </sd-splitter-panel>
    <sd-splitter-panel [size]="1" unit="flex">
      <main>N�"i dung � flex 1 (chiếm toàn b�" không gian còn lại)</main>
    </sd-splitter-panel>
...
@Component({
  ...
  template: `
    <sd-button type="light" color="primary"
               prefixIcon="menu_open" title="Gập / m�x sidebar"
...
<sd-splitter orientation="horizontal">
  <sd-splitter-panel [size]="200" unit="px" [resizable]="false">
    <nav>Sidebar � fixed 200px, không kéo �ược</nav>
  </sd-splitter-panel>
  <sd-splitter-panel [size]="1" unit="flex">
    <main>Content � flex</main>
  </sd-splitter-panel>
  <sd-splitter-panel [size]="300" unit="px" [minSize]="200" [collapsible]="true">
    <aside>Inspector � kéo �ược, gập �ược</aside>
  </sd-splitter-panel>
</sd-splitter>
```

### Anti-patterns
- �R Placing `<sd-splitter-handle>` manually in the template � the container creates them via `createComponent()`; manual handles will be appended but never wired
- �R Using `storageKey` without `panelId` on every panel � restore falls back to index matching, so reordering panels silently corrupts the saved layout
- �R Declaring `unit="flex"` with very large `[size]` (e.g. `1000`) thinking it's percent � flex sizes are normalized weights; `1000` vs `3000` behaves identically to `1` vs `3`
- �R Putting only one `<sd-splitter-panel>` child � no handle is rendered, no resize possible; collapse to a regular `<div>` instead
- �R Reading `getLayout()` mid-drag expecting "live" intermediate sizes � `getLayout()` reads `liveSizes` which DOES update during drag, but `committedLayout` and `(layoutChange)` only fire on commit. If you need true live sizes, hook a custom effect on `liveSizes()`.
- �R Toggling `[disabled]` to "freeze" sizes while still letting the user see the cursor change � disabled sets `pointer-events: none` on handles; the cursor stays default. Use `[resizable]="false"` per-panel if you want some seams locked and others not.
- �R Animating panel widths externally (e.g. via CSS `transition: flex 300ms`) � the splitter writes `style.flex` imperatively on each frame; transitions cause visual lag and the `.sd-splitter--dragging` class already suppresses them mid-drag
...
### Anti-patterns
- �R Setting `style="flex: 1"` or `style="width: 300px"` directly on `<sd-splitter-panel>` � the parent overwrites `style.flex` every frame; your style is lost
- �R Omitting `[panelId]` while using `storageKey` � index-based restore breaks on reorder
- �R Declaring `[minSize]="100"` with `unit="flex"` thinking 100 means pixels � `minSize` shares the panel's unit; a flex panel's `minSize=100` is a flex weight of 100, which clamps essentially never
- �R Putting heavy content in a collapsed panel without `@if` � the DOM is still mounted (flex: 0 0 0), so heavy children (charts, tables) still consume memory. Wrap with `@if (!panelCollapsed())` if needed.
- �R Trying to set `[collapsed]="true"` initially without `[collapsible]="true"` � the model accepts it, but the user has no way to expand later
