...
Floating dev/QA inspector — quét toàn bộ `[data-autoid]` trên page, audit element thiếu/trùng autoid, export danh sách ra CSV / JSON / Markdown POM skeleton, và có thể gọi backend để tạo E2E test ZIP.
...
| `config`  | `SdAutoidInspectorConfiguration \| undefined` | `undefined` | Tuỳ biến scope scan và backend tạo test. `{ host?: string; root?: HTMLElement; extraRequireSelectors?: string[] }`. Mặc định scan `document.body` và tập selector Core UI nội bộ. |

### `SdAutoidInspectorConfiguration`

| Field                   | Type          | Notes                                                                                                                           |
| ----------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `host`                  | `string`      | Base URL của backend `sdcorejs-forge`, ví dụ `http://localhost:3012`. Khi có `host`, tab Export hiển thị nhóm **Tạo E2E test**. |
...
AutoId Inspector dùng tập selector nội bộ sau để audit element thiếu `data-autoid`:
...
Không có `@Output()` event. Consumer có thể dùng `ViewChild(SdAutoidInspector)` để gọi các method public như `openPanel()`, `refresh()`, `exportJson()` hoặc `exportE2eTest()`. Các model dữ liệu scan/audit là chi tiết nội bộ và không được export qua public entrypoint.
...
| `exportE2eTest(target)` | `Promise<void>` | Gọi backend configured qua `host` để tạo ZIP. `target ∈ 'playwright' \| 'robot'`.               |

## Internal services

| Service                    | Purpose                                                                                                                                                                                                                                                                                                    |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SdAutoidScannerService`   | `scan(root, requireSelectors?)` → `AutoidElement[]`. Truyền `requireSelectors` (vd các sd-\* form/button) → thêm phần tử **fallback** cho node khớp selector mà thiếu `data-autoid` (`autoid: ''`, xpath theo vị trí thẻ, `missingAutoid: true` + `warning`). `groupByAutoid(els)` map autoid → element[]. |
| `SdAutoidAuditService`     | `audit(elements, { root, requireSelectors })` → `AutoidAuditResult` (duplicates + missing).                                                                                                                                                                                                                |
| `SdAutoidHighlightService` | `apply(root)` outline xanh / đỏ; `applyMissing(nodes)` dashed cam; `clear(root)` restore inline style gốc.                                                                                                                                                                                                 |
| `SdAutoidExportService`    | `toJson(elements, meta?)` (có `meta` → bọc `{ meta, elements }`), `parseQueryParams(search)`, `toCsv / toMarkdownPom / toMarkdownTable` + `download(content, filename, mime)` + `downloadBlob(blob, filename)` + `copyToClipboard(text)`.                                                                  |

Các service này phục vụ implementation nội bộ của component và không được re-export từ `@sdcorejs/angular/components/autoid-inspector`.
...
- **3 tab segment**:
  ...
  - **Export** — các nút local: `Copy JSON / JSON / CSV / MD POM / MD Table`. Nếu `config.host` hoặc `SD_AUTOID_INSPECTOR_CONFIGURATION.host` có giá trị, hiển thị thêm nhóm **Tạo E2E test** với `Playwright` và `Robot Framework`.
...
### 4. Bật tạo E2E test qua backend forge
...
Khi `host` có giá trị, tab **Export** hiển thị nhóm **Tạo E2E test**. Nút `Playwright` gọi `POST <host>/e2e/test-generator/playwright`, nút `Robot Framework` gọi `POST <host>/e2e/test-generator/robot`, body theo dạng `{ payload: { meta, elements }, options }`, response là file ZIP. API key AI nằm ở backend env, không đặt trên frontend.
...
### E2E ZIP (`exportE2eTest`)

Chỉ bật khi có `host` trong `[config]` hoặc `SD_AUTOID_INSPECTOR_CONFIGURATION`.

```ts
await inspector.exportE2eTest('playwright'); // POST <host>/e2e/test-generator/playwright
await inspector.exportE2eTest('robot'); // POST <host>/e2e/test-generator/robot
```
...
{
  "payload": {
    "meta": { "pageUrl": "/customers/create" },
    "elements": [],
  },
  "options": {
    "language": "vi",
    "mode": "happy-path",
    "outputName": "customer-create-playwright",
    "testName": "Customer Create Playwright",
    "baseUrl": "https://app.example.com",
  },
}
```

Backend trả `application/zip`; component đọc filename từ `Content-Disposition` nếu có, rồi tải file xuống.
...
- `autoid-inspector.component.spec.ts` — 26 case (FAB visibility, togglePanel + fallback count, audit duplicate / missing, highlight ON/OFF, Esc, filter, copyJson `{meta,elements}`, ngOnDestroy cleanup, E2E data-\* attributes, topLevelElements, tableGroups, state chip rendering, E2E test generation via `host`).
