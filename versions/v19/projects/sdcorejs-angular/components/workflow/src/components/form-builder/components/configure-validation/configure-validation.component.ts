...
import { ChangeDetectionStrategy, ChangeDetectorRef, Component, EventEmitter, Inject, Output, ViewChild } from '@angular/core';
...
@Component({
  ...
  imports: [MatMenuModule, SdModal, SdButton, SdInput, SdSelect, ExpressionBuilderComponent, MatIconModule],
})
export class ConfigureValidationComponent {
  ...
  constructor(
    ...
    @Inject(SD_WORKFLOW_CONFIGURATION) private workflowConfiguration: ISdWorkflowConfiguration
  ) {
    this.functions = this.workflowConfiguration?.form?.validation?.functions || [];
  }
