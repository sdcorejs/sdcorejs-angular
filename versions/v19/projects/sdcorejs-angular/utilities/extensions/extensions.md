...
```ts
ArrayUtilities.search(users, 'Đ�', ['fullName', 'email']); // matches "Do" too
```
...
| `convertToSnakeCaseCode` | `(name: string) => string` | `"Đ�"i Kỹ Thuật"` �  `"doi_ky_thuat"`. Throws if `name` not a string. |
