...
pattern for `QB_DATE_MODES` / `QB_VALUE_SOURCE_OPTIONS` / `QB_RELATIVE_UNIT_OPTIONS`.

Prefer the same `computed()` approach over the `translate` pipe for labels that must survive a runtime language
