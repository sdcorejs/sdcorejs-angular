...
**Public to portals**: �R NOT re-exported from the main `@sdcorejs/angular` barrel � Core UI internal only. Portals that need the API import it explicitly from `@sdcorejs/angular/i18n`.
...
## When NOT to use
- �R For dev logs (`console.log/warn/error`) � these are not UX. Leave VI / EN literals as-is and mark the line with `// @i18n-ignore` so the guard script ignores them.
- �R For code comments � they are not UX.
- �R For non-UI runtime data such as diacritic-stripping regex literals (`/[àáả...ư]/g`) � those are data. Whitelist the FILE in `scripts/check-i18n.mjs`.
- �R For dynamic database content (entity names, user-provided text) � translate those at the data source, not via i18n keys.
- �R For `export const` arrays evaluated at module-eval time (no DI available). Three options:
...
### `npm run check:i18n`
Scans `projects/sdcorejs-angular/**/*.{ts,html}` for raw Vietnamese diacritics (the regex covers all combinations of `ì-ỹ` + `Đ`/`�`). Fails CI on any hit outside the whitelist.
...
console.log('--- Bắt �ầu chế ��" theo dõi mạng ---');
```
...
const SD_UPLOAD_MESSAGES = {
  vi: { 'invalid-format': '[{name}] File tải lên không �úng ��9nh dạng. Vui lòng chọn lại', /* ... */ },
  en: { 'invalid-format': '[{name}] Invalid file format. Please select again', /* ... */ },
  ja: { 'invalid-format': '[{name}] �"����㒫形式�R正�く��`ま�:���� 丬度選�`~�てくだ�"�', /* ... */ },
  ko: { 'invalid-format': '[{name}] �RR일 ��"�9�이 �ܬ�르짬 �"`�`��9��9�. �9��9S ��풝�"� 주���a', /* ... */ },
  zh: { 'invalid-format': '[{name}] �!件格式不正确�R请�!����0�9�', /* ... */ },
} as const;
...
Trade-offs:
- �S& No DI dependency � works in any context (utility scripts, web workers, ⬦).
- �S& `Error.message` is already translated � consumer just calls `notify.error(err.message)`.
- �S& Auto-sync with `I18nService` because both read the same `'sd-core.language'` localStorage key.
- �R Strings are duplicated outside the central catalog � `check:i18n-parity` does NOT validate them.
- �R File-level `WHITELIST` entry in `scripts/check-i18n.mjs` required (otherwise the hardcode guard flags the diacritics).
- Use only when DI is genuinely unavailable. Prefer the standard `I18nService.t()` path otherwise.

## Anti-patterns

- �R **Hardcoding VI/EN strings in components/services/templates** � always go through `i18n.t()` or `| translate`. The `check:i18n` guard catches it at CI.
- �R **Translating `console.log/warn/error` arguments** � they're dev logs, not UX. Mark with `// @i18n-ignore`.
- �R **Translating code comments / JSDoc** � comments are documentation, not user content.
- �R **Calling `setLanguage()` mid-render expecting the UI to update without reload** � pipe is `pure: true`. Use the default reload behavior, or accept that bindings already rendered won't refresh.
- �R **Adding a new key to only `EN_MESSAGES`** � TypeScript will error in `VI_MESSAGES`/`JA_MESSAGES`/`KO_MESSAGES`/`ZH_MESSAGES`; CI parity check will fail too. Add to ALL catalogs in the same commit.
- �R **Inventing new prefixes outside `core.*`** � portal-level keys shouldn't be added to Core's catalogs. Portals manage their own catalog. (Core's i18n is for Core's own strings; portals may use a separate translation system or wire portal strings via factory functions.)
- �R **Async custom language** � `CustomLanguageProvider` is sync. If you need to fetch a catalog, resolve it during `APP_INITIALIZER` before bootstrap and pass the resolved object via `useFactory`.
- �R **Using `String(params[x])` with possibly-undefined values directly in the template** � coalesce upstream: `{ name: user?.fullName ?? '' }`. Otherwise the placeholder renders as `"undefined"` at runtime.
- �R **Translating values in a top-level `export const` array** � runs at module-eval, no DI. Three options:
...
- �R **Calling `i18n.t()` once and caching the result for the page lifetime** � that's fine on its own, but means a language toggle WON'T update that cached value (the page reload via `setLanguage()` should reset it anyway, but be aware).
