...
| `size`            | `Size` (`'sm' \| 'md' \| 'lg'`)                           | `'md'`                                      | Field height (applied via `.sd-md` / `.sd-sm` class). Use `size="sm"` inside `<sd-table>` filters/cells or other dense table UI.                                                                                                                                                                                                                                                                                                                                       |
...
<sd-textarea label="Ghi chú" [(model)]="model.note"> </sd-textarea>

<!-- 2. Reactive FormGroup (self-registers via addControl) -->
<form [formGroup]="form">
  <sd-textarea label="Mô tả" name="description" [form]="form" required [maxlength]="500" [(model)]="model.description"> </sd-textarea>
</form>

<!-- 3. Template-driven NgForm -->
<form #f="ngForm">
  <sd-textarea label="Lý do" name="reason" [form]="f" required [(model)]="model.reason"> </sd-textarea>
</form>
```
...
Every standalone host that uses `<sd-textarea>` must import `SdTextarea`. Projected label/view templates require their directives in `imports`.
...
@Component({
  standalone: true,
  imports: [SdTable, SdTableCellDefDirective, SdTextarea, SdLabelDefDirective, SdViewDefDirective],
  template: `
...
```html
<sd-textarea [form]="form" name="description" label="Mô tả" required [maxlength]="500" [(model)]="model.description"> </sd-textarea>
```

### 2. Auto-growing comment box

```html
<sd-textarea [form]="form" name="comment" label="Ghi chú" autoHeight rows="3" placeholder="Nhập ghi chú..." [(model)]="model.comment">
</sd-textarea>
```
