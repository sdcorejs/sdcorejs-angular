...
## One-line purpose
Radio-button group � user picks exactly ONE option from a small, fixed list. Items can be laid out inline (`row`) or stacked (`column`). Use when the full set of choices should be visible at once (�0� ~6 options); for longer lists, use `<sd-select>` instead.
...
**Pattern**: `SdRadio` không implement `ControlValueAccessor`. Dùng `[form]` + `name` �Ồ �Ēng ký control vào FormGroup cha. Trên `ngAfterViewInit`, component tự gọi `formGroup.addControl(name, formControl)` và `removeControl` khi destroy.

- **`formControlName` và `[(ngModel)]` KH�NG �ược h� trợ.** Dùng `[(model)]` cho two-way binding và `[form]+[name]` cho FormGroup integration.
- **`[viewed]="true"`** chuyỒn sang DETAIL read-only mode: radio group b�9 ẩn, giá tr�9 hiỒn th�9 là text thuần (hoặc link nếu có `hyperlink`).
...
```
Dùng `required` bare attribute �Ồ bắt bu�"c chọn; giá tr�9 pre-selected truyền qua `[(model)]` thay vì kh�xi tạo trong FormGroup.

### 2. Stacked option list (column display)
```html
<sd-radio
  [form]="form" name="approvalMode"
  label="Hình thức duy�!t"
  display="column"
  [items]="approvalModes"
...
```typescript
approvalModes = [
  { id: 1, label: 'Duy�!t tự ��"ng' },
  { id: 2, label: 'Duy�!t thủ công' },
  { id: 3, label: 'Duy�!t theo lu�ng' },
];
onModeSelected(event: { value: any; item?: any }) {
  console.log('Selected item:', event.item);
}
```
`display="column"` xếp chọn theo chiều dọc; `sdSelection` trả về cả `value` lẫn `item` object �Ồ tránh look-up thủ công.
...
customerTypes = [
  ...
  { code: 'BIZ', name: 'Doanh nghi�!p' },
];
```
Khi `[viewed]="true"`, radio group b�9 ẩn và giá tr�9 hiỒn th�9 dư�:i dạng text thuần hoặc link � phù hợp cho màn hình xem chi tiết (DETAIL mode).

## Form integration (3 cách)

### Cách 1: Template-driven `[(model)]` (không dùng FormGroup)
Dùng khi ch�0 cần two-way bind giá tr�9, không cần reactive validation từ FormGroup cha.
...
statusOptions = [
  ...
  { value: 'inactive', label: 'Không hoạt ��"ng' },
];
...
Dùng khi radio là m�"t phần của form l�:n hơn � `SdRadio` tự `addControl` vào FormGroup khi `ngAfterViewInit`.
...
Dùng khi toàn b�" form dùng `#f="ngForm"` � truyền tham chiếu NgForm; component tự unwrap sang `NgForm.form` (`FormGroup` bên trong).
...
�R **Dùng `<sd-radio>` cho nhiều hơn ~6 lựa chọn hoặc danh sách tải từ API**
```html
<!-- Đừng làm vậy -->
...
```
`<sd-select>` h� trợ scroll, filter, lazy-load � t�t hơn khi list dài.

---

�R **Dùng `formControlName` hoặc `[(ngModel)]`**
```html
<!-- Đừng làm vậy -->
...
�R **Quên `valueField` hoặc `displayField`**
```html
<!-- Đừng làm vậy � sẽ throw l�i runtime -->
...
```
Cả hai �ều là `required: true` inputs � thiếu m�"t trong hai sẽ gây l�i Angular compiler.

---

�R **Dùng `[disabled]="true"` �Ồ biỒu th�9 trạng thái �ọc (DETAIL state)**
```html
<!-- Đừng làm vậy -->
...
```
`[viewed]="true"` ẩn radio group và hiỒn th�9 text/link � �úng semantic cho màn hình xem chi tiết.

---

�R **Tr�"n nhiều kiỒu dữ li�!u trong `items`**
```typescript
// Đừng làm vậy
items = ['active', { code: 'inactive', name: 'Không hoạt ��"ng' }];
```
Thay vào �ó hãy dùng:
```typescript
items = [
  { code: 'active', name: 'Hoạt ��"ng' },
  { code: 'inactive', name: 'Không hoạt ��"ng' },
];
```
