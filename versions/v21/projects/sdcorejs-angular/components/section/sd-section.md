...
**Type**: Component (two related components, documented together â€” they are typically used as a pair)
...
### One-line purpose
Card-style content panel with a header (icon + title + sub-title), optional collapse toggle, and a slot-based body â€” the standard wrapper for grouping fields, tables, or any sub-section on detail / form pages.

### When to use
- Any "block" of related content on a detail page (`ThÃ´ng tin chung`, `ThÃ´ng tin liÃªn há»‡`, `Lá»‹ch sá»­`, ...)
...
### When NOT to use
- When you need a slide-out panel â†’ use `<sd-side-drawer>`
- When you need a modal / dialog â†’ use `<sd-modal>`
- For a single `label : value` row â†’ use `<sd-section-item>` directly
- For a hint + action row â†’ use `<sd-quick-action>`
- When you need tabs â†’ use `<sd-tab-router>`
...
| `collapsed` | `boolean` (model â€” two-way) | `false` | Two-way bindable via `[(collapsed)]`. When true and `collapsable` is true, hides the body. |
...
| `[sdHeaderRight]` | Right-side header content â€” typically `<sd-button>`s ("Edit", "Add", filter chips). Sits left of the chevron when collapsable. |
...
### Behaviors / quirks
- Clicking the header calls `toggleCollapse()` â€” but only flips `collapsed` if `collapsable()` is true (otherwise it forces `collapsed` to `false`)
...
<sd-section
  ...
  subTitle="ThÃ´ng tin cÆ¡ báº£n cá»§a nhÃ¢n viÃªn"
  [collapsable]="true">
  <sd-section-item label="Há» vÃ  tÃªn">Nguyá»…n VÄƒn A</sd-section-item>
  <sd-section-item label="Email">a.nv@onemount.com</sd-section-item>
  <sd-section-item label="PhÃ²ng ban">Sales</sd-section-item>
</sd-section>
```

#### 2. Section with right-side action
```html
<sd-section icon="group" title="ThÃ nh viÃªn dá»± Ã¡n">
  <sd-button sdHeaderRight
    type="fill" color="primary" size="sm" prefixIcon="add"
    title="ThÃªm thÃ nh viÃªn" (click)="onAddMember()">
  </sd-button>
  <sd-table [data]="members" [columns]="memberCols"></sd-table>
</sd-section>
```

#### 3. Section wrapping a table â€” full width, no padding
```html
<sd-section title="Lá»‹ch sá»­ thao tÃ¡c" [noPaddingBody]="true">
...
<sd-section [hideHeader]="true">
  <p>Ná»™i dung tá»± do, khÃ´ng cÃ³ header. Card váº«n cÃ³ padding 16px vÃ  bo gÃ³c.</p>
</sd-section>
```
...
### Anti-patterns
- âŒ Nesting `<sd-section>` inside `<sd-section>` â€” visually heavy; prefer flat structure with multiple sibling sections
- âŒ Setting `collapsed="true"` without `collapsable="true"` â€” the section will be expanded and never collapsible (the toggle silently re-opens it)
- âŒ Adding margin to the host instead of letting the page layout handle spacing â€” sections are designed to stack with parent `gap-*`
- âŒ Placing `<sd-section-item>` outside `<sd-section>` â€” it works but looks orphaned; design intent is grouped rows
- âŒ Using as a modal substitute â€” modals need backdrop, focus-trap, escape; use `<sd-modal>`
...
### One-line purpose
A single `label : value` row meant to live inside a `<sd-section>` body â€” used for compact info display on detail pages.
...
### When NOT to use
- For editable inputs â€” use a labeled `<sd-input>`, `<sd-select>`, etc. directly inside the section
- For message + action rows â†’ use `<sd-quick-action>`
- As a standalone row outside any `<sd-section>` â€” it works, but it looks orphaned without the card wrapper

### Inputs
| Name | Type | Default | Notes |
| --- | --- | --- | --- |
| `label` | `string` (REQUIRED, signal input) | â€” | The left-column label text. Rendered with `T14R text-black400` typography. |
| `labelWidth` | `string` | `'150px'` | Fixed width of the label column. Falsy values (`null`, `undefined`, `''`) coerce back to `'150px'`. |

> **Coerce note**: `labelWidth` transform: `(val) => val || '150px'` â€” any falsy value resets to the default.
...
- Label typography: `T14R text-black400` â€” 14px regular, muted 400-tone gray
- Value area: `flex: 1`, inherits parent typography; no padding imposed â€” value content controls its own spacing
- No border between items â€” spacing between rows comes from the parent container (`<sd-section>` body)

### Behaviors / quirks
- `label` is a required signal input â€” Angular will throw `NG0950` at runtime if omitted
- `labelWidth` coerces falsy values (`null`, `undefined`, empty string) back to `'150px'` automatically
- The component has no collapse, loading, disabled, or selection state â€” it is purely presentational

### Examples
```html
<sd-section title="ThÃ´ng tin liÃªn há»‡">
  <sd-section-item label="Email">a.nv@onemount.com</sd-section-item>
  <sd-section-item label="Sá»‘ Ä‘iá»‡n thoáº¡i">0901 234 567</sd-section-item>
  <sd-section-item label="PhÃ²ng ban" labelWidth="200px">
...
### Anti-patterns
- âŒ Forgetting `label` â€” it is required and Angular will throw at runtime
- âŒ Mixing `<sd-section-item>`s with very different `labelWidth`s in the same section â€” labels won't visually align
- âŒ Using for editable form rows â€” section-item is a read-only display pattern; for forms use a labeled `<sd-input>` directly
- âŒ Passing a reactive expression as `label` without using `[label]` binding syntax â€” `label` is a signal input, not a plain attribute

---

## Related
- `<sd-quick-action>` â€” single message + action row (different pattern; sits inside section bodies)
- `<sd-modal>`, `<sd-side-drawer>` â€” when you need an overlay instead of an inline card
- `<sd-tab-router>` â€” when sections should be split across tabs
