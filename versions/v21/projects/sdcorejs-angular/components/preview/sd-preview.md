...
- Carousel with dots indicator for �0� 8 images (use `thumbnailPosition="dots"`)
...
| **G** | Broken image (per-image error) | **Inline placeholder INSIDE the image area** � 96�96 rounded-24 danger tile with `broken_image` icon + "Không tải �ược ảnh" + retry button (`refresh` icon, accent background). Nav arrows, dots, thumbnails, and the floating toolbar **stay visible and interactive** so the user can navigate to another image. Thumbs of broken items show a small `broken_image` icon on a translucent red background. |
...
```html
<sd-modal #modal title="Xem hợp ��ng" width="92vw" height="86vh">
  <sd-preview-pdf
    [source]="modalSrc()"
    (close)="modal.close()">
  </sd-preview-pdf>
