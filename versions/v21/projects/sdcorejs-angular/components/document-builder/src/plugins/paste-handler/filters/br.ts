 ...
 * @internal
 */
export function transformBlockBrsToParagraphs(documentFragment: ViewDocumentFragment, writer: ViewUpcastWriter): void {
...
function findSibling(
  ...
  { blockElements, inlineObjectElements }: { blockElements: string[]; inlineObjectElements: string[] }
) {
  ...
  position = position.getLastMatchingPosition(
    ({ item }) => item.is('element') && !blockElements.includes(item.name) && !inlineObjectElements.includes(item.name),
    { direction }
  );
 ...
 */
function isBlockViewElement(node: ViewNode | null, blockElements: string[]) {
