...
- �R Using `type="number"` for VND amounts � use `<sd-input-number>` for proper thousand-separator formatting.
- �R Wiring up trim logic in the parent � the component already trims on blur/Enter.
- �R Hard-coding regex for common patterns � check `VALIDATION_PATTERNS` first (`EMAIL`, `PHONE`, `TAX_CODE`, ⬦) so error messages stay consistent.
- �R Hand-rolling a "clear" suffix via `sdSuffixDef` � a hover-gated clear button is now **built in** (auto-shows when there's a value and the field is editable). Use `sdSuffixDef` only for additional affordances (search, password toggle, swatch ⬦).
