...
| ------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `clear($event?)`    | `(Event?) => void` | Clears the value (`null`), emits `sdChange` + `sdSelection`. No-op **only** when the value is already `null` / `undefined` / `''` — a selected `0` (or any other falsy-but-real value) IS cleared. Writes to `formControl` with events so validators re-run and the message updates. |
| `focus()`           | `() => void`       | Focuses the input and opens the suggestion panel.                                                                                                                                                                                                        |
| `blur()`            | `() => void`       | Blurs the input.                                                                                                                                                                                                                                         |
| `open()`            | `() => void`       | Opens the panel programmatically (no-op when disabled).                                                                                                                                                                                                  |
| `enterInlineEdit()` | `() => void`       | Opens the panel from the `viewed='inline'` text face.                                                                                                                                                                                                     |
| `reValidate()`      | `() => void`       | Re-runs validators on `formControl` — the control that actually carries `required` / `[validator]` / `inlineError` — with events, so the error message refreshes.                                                                                         |
...
- **Reactive validator updates are ADDITIVE** — `required`, `validator`, and `inlineError` are signal inputs routed through the shared form-control connector, which applies them with `addValidators` / `removeValidators` (never `setValidators` / `setAsyncValidators`). You can flip them on/off at runtime with no manual call needed, **and a validator you attach yourself to the exposed `formControl` survives those changes**. Previously an internal `effect()` called `setValidators` + `setAsyncValidators`, which replaced the whole list and silently wiped consumer-attached validators every time one of the three inputs changed.
...
- **`onSelect` and `clear()` mirror the value to `formControl` WITH events** (no `{ emitEvent: false }`). Deliberate: `formControl` carries the async `[validator]`, and `errorMessage` is a `computed` that only recomputes when the control's reactive snapshot (`sdFormControlState`) ticks on a control event. Suppressing the event would let the async validator's `setErrors(...)` completion run silently → no tick → the `[validator]` message never appears or never clears (red outline, no text). The `formControl.valueChanges` subscriber writes `valueModel` (guarded by `!==`) and both call sites re-set the same value (no-op), so emitting causes no loop. (`onSelect` fixed 2026-06-05; `clear()` fixed in the same sweep afterwards.)
- **`clear()` treats `0` as a real value.** The guard is `value != null && value !== ''`, not a truthiness check — the value-resolution path already accepts `0` (`if (val || val === 0)`), so a truthiness guard would silently refuse to clear an item whose value is `0`.
- **`reValidate()` targets `formControl`, not `inputControl`.** `inputControl` only holds the search text and carries no validators; the public re-validate API used to run against it and therefore did nothing.
...
- The "add new" row inside the panel is a real `<button type="button">` (it used to be a
  `<div (click)>`: no role, no keyboard activation).
