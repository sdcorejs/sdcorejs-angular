...
## One-line purpose
Dark-themed, fullscreen-capable image gallery viewer â€” opens a modal showing a stage image with zoom / rotate / download, plus a flexible thumbnail strip (bottom / right / left / top / dots / none) so users can flip through a list of files or CDN URLs.
...
- Carousel-style preview for â‰¤ 8 images (use `thumbnailPosition="dots"`)

## When NOT to use
- For PDFs, Word, Excel, or other non-image documents â†’ not supported (non-image `File`s are silently dropped)
- For inline embeds in the page body â†’ just use a plain `<img>` or `<sd-document-builder>`
- When you need annotation / cropping â†’ this is a passive viewer
- When you need to show > 200 images at once â†’ the component loads each in parallel; consider lazy pagination upstream
...
| **G** | Error (`stage = 'error'`) | 96Ã—96 rounded-24 danger tile with `broken_image` icon + "KhÃ´ng táº£i Ä‘Æ°á»£c áº£nh" + retry button (`refresh` icon, accent background). |
...
- **Nav arrows**: 48Ã—48 round, `rgba(20,20,22,0.6)` + `backdrop-filter: blur(10px)`, `chevron_left` / `chevron_right` icons, disabled state at 35% opacity
...
- **Keyboard hint**: top-left of stage, `<kbd>`-style chips for â† â†’ Esc
...
<sd-button (click)="preview.open([row.avatarUrl])">Xem áº£nh Ä‘áº¡i diá»‡n</sd-button>
```
...
const items: PreviewItem[] = projectPhotos.map(p => ({
  url: p.cdnUrl,
  name: p.fileName,
  caption: p.takenAt + ' Â· ' + p.locationName,
  alt: p.altText,
}));
...
### 5. Carousel-style with dots indicator (â‰¤ 8 images)
...
- âŒ Creating one `<sd-preview-image>` per row â€” declare ONE in the page and call `.open(...)` per click.
- âŒ Passing PDF / non-image URLs â€” fetched as `<img>` they show as broken; non-image `File`s are dropped silently.
- âŒ Trying to bind `[images]` directly â€” there is no `images` input, always use `open(...)`.
- âŒ Calling `open()` from inside `ngOnInit` of a component that may be destroyed before the modal opens â€” the in-flight blob fetches will still resolve and leak handles. Gate on user interaction.
- âŒ Reading `activeImage()` from outside the template â€” it's a computed signal; use the `activeIndexChange` output instead.
- âŒ Mixing `thumbnailPosition="dots"` with > 20 images â€” dots become illegible. Switch to `bottom`.

## Related
- `<sd-modal>` â€” chrome host (backdrop + focus trap + escape-to-close)
- `<sd-document-builder>` â€” for PDFs / Word / Excel previews
- `<sd-button>` â€” used by callers to trigger `.open(...)`
