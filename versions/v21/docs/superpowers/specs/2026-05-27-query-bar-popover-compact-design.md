...
1. The chip popover is tall: a header row (icon + field), then a separate **"Điều kiện"** labelled row for the operator, then a separate **"Giá trị"** labelled row for the value, then an **"Áp dụng"** footer button. Lots of vertical space and an explicit apply step.
...
- **A.** Compact popover: operator moves onto the header row (right-aligned, vertically centered, no "Điều kiện" label); drop the "Giá trị" label; remove the "Áp dụng" button and auto-apply staged edits when the popover closes (commit only — the Search button still runs the query).
...
- Right: the `sd-operator` operator picker, pushed right with `margin-left: auto`, shown only when `editingShowOperatorSelector()` is true (field exposes >1 operator). The **"Điều kiện"** section label is removed.
...
- Remove the **"Điều kiện"** and **"Giá trị"** `c-pop-section-label` rows.
...
- Add a placeholder so the control is self-explanatory (no label needed):
  - text / number / `sd-input` → `placeholder="Nhập giá trị"`.
  - `values` / `lazy-values` `sd-select`, `date`, `datetime` → `placeholder="Chọn giá trị"`.
...
- **Popover header:** when a chip with a multi-operator field is open, `sd-operator` renders inside `.c-pop-header`; no `.c-pop-section-label` with "Điều kiện"/"Giá trị" exists in the popover.
