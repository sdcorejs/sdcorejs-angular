...
matches your installed package (`@sdcorejs/angular@20.0.5` → `/docs/20.0.5/`).

The archive is generated by `scripts/collect-release-docs.mjs` from the tag publish workflow.
It creates `published-docs/19.<patch>/`, `published-docs/20.<patch>/`, and
`published-docs/21.<patch>/` only after npm publish succeeds; `deploy-pages.yml` then copies
