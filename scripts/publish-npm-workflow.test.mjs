...
test('release workflow delegates the validated four-target plan to one sequential publisher', () => {
  ...
  has(publishInvocations[0], /--suffix\s+["']?2\.14["']?/u);
  has(publishInvocations[0], /--baseline-suffix\s+["']?2\.13["']?/u);
...
test('every release entry path requires the immutable v2.14 tag to point at main', () => {
  ...
  has(
    verifySource.source,
    /ref:\s*\$\{\{\s*github\.event_name == 'workflow_dispatch' && 'refs\/tags\/v2\.14' \|\| github\.ref\s*\}\}/u,
...
test('all four packages are built and verified as immutable artifacts before publication', () => {
  ...
  has(verifierCommands, /--baseline-suffix\s+["']?2\.13["']?/u);
...
test('manual recovery retains the original tarballs without repacking them', () => {
  ...
  has(recoveryStep, /name:\s*sdcorejs-angular-\$\{\{ matrix\.version \}\}-2\.14/u);
...
test('postpublish materializes verified v19, clean-installs Showcase and commits docs plus page retention', () => {
  ...
  has(postpublishCommands, /npm run build:page -- --suffix ["']?2\.14["']?/u);
  has(postpublishCommands, /published-pages\/2\.14/u);
  ...
  assertExactCommandSequence(postpublish.source, [
    'git commit -m "docs: publish Angular 22 release 2.14"',
...
test('publisher revalidates exact versions, recovery tags, latest and provenance through the tested transaction', () => {
  const publisher = oneJobMatching(/release-package-contract\.mjs[^\r\n]*--publish\b/u, 'publisher');
  const publisherCommands = executableCommands(publisher.source);
  has(publisherCommands, /--suffix\s+["']?2\.14["']?/u);
  has(publisherCommands, /--baseline-suffix\s+["']?2\.13["']?/u);
