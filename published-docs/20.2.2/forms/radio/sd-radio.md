...
Every standalone host that uses `<sd-radio>` must import `SdRadio`. Projected label/suffix/view directives must also be imported if used.
...
@Component({
  standalone: true,
  imports: [SdTable, SdTableCellDefDirective, SdRadio, SdLabelDefDirective, SdSuffixDefDirective, SdViewDefDirective],
  template: `
    <sd-table [option]="tableOption">
      <ng-template sdTableCellDef="type" let-row>
        <sd-radio display="row" [items]="typeList" valueField="code" displayField="name" [(model)]="row.type"> </sd-radio>
      </ng-template>
    </sd-table>
  `,
})
...
`sd-radio` does not expose `hideInlineError`. Prefer `<sd-select size="sm" hideInlineError>` for editable table cells when the option list is long or validation error text would expand the row.
...
<mat-radio-group data-autoid="forms-radio-gender" data-disabled="false" data-empty="false" data-value="M">
  <!-- ... -->
</mat-radio-group>
```
