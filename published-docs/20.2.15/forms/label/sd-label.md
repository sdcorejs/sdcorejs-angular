...
- The info icon is small (1rem × 1rem), `info` from the `material-icons-outlined` font set; tooltip appears below on hover
...
<div class="form-field">
  <sd-label label="Số dư khả dụng" helperText="Đã trừ các giao dịch đang chờ duyệt"> </sd-label>
...
```html
<sd-label label="Mã khách hàng" description="Tự động sinh nếu để trống" required> </sd-label>
```
