...
Release đang chuẩn bị: `v2.9` → `19.2.9` / `20.2.9` / `21.2.9` / `22.2.9`.
Workflow `publish-npm.yml` pin đúng tag `v2.9`; snapshot `scripts/release-contracts/2.9.json`
đối chiếu từng line với bản `*.2.8` cùng Angular major. Fallback từ Angular 22 sang 21
...
- Push tag `v<release-suffix>` → tạo đúng bốn version `19.<suffix>` / `20.<suffix>` / `21.<suffix>` / `22.<suffix>`.
  - `v2.9` → `19.2.9`, `20.2.9`, `21.2.9`, `22.2.9`.
...
**Tag stable 19.2.9/20.2.9/21.2.9/22.2.9 sau khi release commit đã merge vào main**:
```bash
git tag v2.9
...
powershell -ExecutionPolicy Bypass -File ./scripts/deploy.ps1 `
  -PatchVersion "2.9" `
  -OutputPath "$env:TEMP\sdcorejs-angular-2.9" `
  -DryRun
```
