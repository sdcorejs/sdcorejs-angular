...
Release đang chuẩn bị: `v2.10` → `19.2.10` / `20.2.10` / `21.2.10` / `22.2.10`.
Workflow `publish-npm.yml` pin đúng tag `v2.10`; snapshot `scripts/release-contracts/2.10.json`
đối chiếu từng line với bản `*.2.9` cùng Angular major. Fallback từ Angular 22 sang 21
...
- Push tag `v<release-suffix>` → tạo đúng bốn version `19.<suffix>` / `20.<suffix>` / `21.<suffix>` / `22.<suffix>`.
  - `v2.10` → `19.2.10`, `20.2.10`, `21.2.10`, `22.2.10`.
...
**Tag stable 19.2.10/20.2.10/21.2.10/22.2.10 sau khi release commit đã merge vào main**:
```bash
git tag v2.10
...
powershell -ExecutionPolicy Bypass -File ./scripts/deploy.ps1 `
  -PatchVersion "2.10" `
  -OutputPath "$env:TEMP\sdcorejs-angular-2.10" `
  -DryRun
```
