...
Release đang chuẩn bị: `v2.13` → `19.2.13` / `20.2.13` / `21.2.13` / `22.2.13`.
Workflow `publish-npm.yml` pin đúng tag `v2.13`; snapshot `scripts/release-contracts/2.13.json`
đối chiếu từng line với bản `*.2.12` cùng Angular major. Fallback từ Angular 22 sang 21
...
- Push tag `v<release-suffix>` → tạo đúng bốn version `19.<suffix>` / `20.<suffix>` / `21.<suffix>` / `22.<suffix>`.
  - `v2.13` → `19.2.13`, `20.2.13`, `21.2.13`, `22.2.13`.
...
**Tag stable 19.2.13/20.2.13/21.2.13/22.2.13 sau khi release commit đã merge vào main**:
```bash
git tag v2.13
...
powershell -ExecutionPolicy Bypass -File ./scripts/deploy.ps1 `
  -PatchVersion "2.13" `
  -OutputPath "$env:TEMP\sdcorejs-angular-2.13" `
  -DryRun
```
