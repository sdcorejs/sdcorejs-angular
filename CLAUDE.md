...
- Push tag `v<release-suffix>` → tạo đúng bốn version `19.<suffix>` / `20.<suffix>` / `21.<suffix>` / `22.<suffix>`.
  - `v2.6` → `19.2.6`, `20.2.6`, `21.2.6`, `22.2.6`.
...
**Tag stable 19.2.6/20.2.6/21.2.6/22.2.6 sau khi release commit đã merge vào main**:
```bash
git tag v2.6
...
powershell -ExecutionPolicy Bypass -File ./scripts/deploy.ps1 `
  -PatchVersion "2.6" `
  -OutputPath "$env:TEMP\sdcorejs-angular-2.6" `
  -DryRun
```
